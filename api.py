...
def write_query_log(query_json, plan_json, pg_time_ms,
    ...
    try:
        ...
        """, (
            ...
            array_time_ms,
            total_time_ms,
            "{" + ",".join(str(x) for x in result_shape) + "}",
            n_files_pruned,
        ))
        conn.commit()
        cur.close()
...
async def legacy_query(time: str, lat_min: int, lat_max: int,
    ...
    try:
        with tiledb.DenseArray("tiledb_store/pam_co", mode="r") as A:
            all_steps = sorted(qi.metadata.all_time_steps('COCL'))
...
@app.get("/ims/health")
async def health():
    steps = qi.metadata.all_time_steps('COCL')
    return {
        "status": "ok",
...
async def time_steps():
    return {"time_steps": sorted(qi.metadata.all_time_steps('COCL'))}
...
async def ims_explain(body: QueryBody):
    ...
    """
    query_dict = body.dict(by_alias=True)
    try:
